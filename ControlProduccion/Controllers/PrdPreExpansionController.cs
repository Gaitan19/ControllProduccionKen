        ...
        public async Task<ActionResult> Edit(PrdPreExpansionViewModel model)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                var vm = new PrdPreExpansionViewModel
                {
                    ...
                    AprobadoSupervisor = modelDto.AprobadoSupervisor,
                    AprobadoGerencia = modelDto.AprobadoGerencia,
                    PreDetPrdPreExpansions = modelDto.PreDetPrdpreExpansions?.Select(p => new PreDetPrdPreExpansionViewModel
                    {
                        Id = p.Id,
                        PrdpreExpansionId = p.PrdpreExpansionId,
                        MarcaTipo = p.MarcaTipo,
                        CodigoSaco = p.CodigoSaco,
                        Lote = p.Lote,
                        FechaProduccion = p.FechaProduccion,
                        DetPrdPreExpansions = p.DetPrdpreExpansions?.Select(x => new DetPrdPreExpansionViewModel
                        {
                            Id = x.Id,
                            PreDetPrdpreExpansionId = x.PreDetPrdpreExpansionId,
                            Hora = x.Hora,
                            NoBatch = x.NoBatch,
